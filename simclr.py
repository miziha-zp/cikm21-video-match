...
history_model_path = 'checkpoint/time_10:12:16:46pytorch_model.bin.align_tag_ich.0'#'checkpoint/time_10:11:21:25pytorch_model.bin.align_tag_ich.6'
...
#     score = test_spearmanr(frame_out_embedding, annotation_file)
#     cprint(f'\nepoch=>{epoch}: frame_out_embedding_spearmanr:{score}', 'blue')   
def get_random_p():
    return np.random.rand()

def random_drop_frame(frames_oral, drop_rate=0.2):
#     frame  batch * 32 * 1536
    # random set frame 0 
    frames = frames_oral.clone()
    for i in range(32):
        p = get_random_p()
        if p < drop_rate:
            frames[:,i:,] = 0
    # set head of frames 0
    p = get_random_p()
    if p < drop_rate:
        frames[:,:10,] = 0
    # set tail of frames 0
    p = get_random_p()
    if p < drop_rate:
        frames[:,-10:,] = 0
    # no action 
...
#         frame_input1 = random_drop_frame(frame_input)
        frame_input2 = random_drop_frame(frame_input)
        
        frame_out, bert_out, concat_emb, tag_pred, \
            bert_out_ich, frame_out_ich, ich_out1 = \
            NET(frame_input, input_id, mask)
        
        frame_out2, _, _, _, _, _, ich_out2 = \
            NET(frame_input2, input_id, mask)
#         print(tag.shape)
        loss_tag = BCELOSS(tag_pred, tag) * TAGSIZE
        match_loss = MATCHLoss(bert_out_ich, frame_out_ich)
        align_loss = ALIGNLoss(frame_out, frame_out2) # +  ALIGNLoss(ich_out1, ich_out2)
        
        loss =  align_loss #+ loss_tag + match_loss +
...
#         break
        if step % 100 == 0:
            now = get_str_time()
            print(f"{now}: epoch:{epoch} \t , step:{step}\t loss:{loss.item()}, match_loss:{match_loss.item()}, loss_tag:{loss_tag.item()} align_loss:{align_loss.item()}")
        if step % 500 == 100:    
