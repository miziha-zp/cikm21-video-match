...
class NetVLADConsensus(nn.Module):
    def __init__(self,
                 num_clusters,
                 alpha,
                 feature_size,
...
class TransformerFrame(nn.Module):
    def __init__(self, representation_cfg) -> None:
        super().__init__()
        self.frame_net = nn.TransformerEncoder(nn.TransformerEncoderLayer(
            d_model=representation_cfg['d_model'],
            nhead=representation_cfg['nhead'],
            dim_feedforward=representation_cfg['dim_feedforward'],
            dropout=representation_cfg['dropout']), num_layers=representation_cfg['num_layers'])
    
    def forward(self, frame):
        out = self.frame_net(frame)
        return out
...
if __name__ == '__main__':
    frame_net = NeXtVLAD(base_config)
    frame_input_tensor = torch.rand(128, 32, 1536)
    frame_out = frame_net(frame_input_tensor)
